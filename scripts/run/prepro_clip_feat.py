...
def extract_visual_features_pipeline(args):
    ...
    processor = CLIPProcessor.from_pretrained(args.model_id, use_fast=True)
...
def main():
    ...
    parser.add_argument(
        "--model_id",  # option
        default="./models/models--openai--clip-vit-large-patch14",
    )
    parser.add_argument(
        "--dataset",
        default="ccmc",
    ...
    parser.add_argument(
        "--output",  # option
        default=Path("./data/ccmc/features/clip_vit_large_patch14_l24"),
    )
